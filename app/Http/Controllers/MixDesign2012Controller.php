...
class MixDesign2012Controller extends Controller {
 ...
 private function saveDesign(Request $r,MixDesign2012Calculator $calc,bool $combined){
  $d=$r->validate(['workflow_id'=>'nullable|integer','project_id'=>'required|exists:projects,id','work_date'=>'required|date','data'=>'required|array','data.*'=>'nullable|numeric','data.construction_type'=>'nullable|integer|between:0,6','data.slump_min'=>'required|numeric|min:0','data.slump_max'=>'required|numeric|gte:data.slump_min','data.slump_design'=>'required|numeric|gte:data.slump_min|lte:data.slump_max','data.trial_cylinder_diameter_mm'=>'required|numeric|min:1','data.trial_cylinder_height_mm'=>'required|numeric|min:1','data.trial_cylinder_count'=>'required|integer|min:1','data.combined_fine_percent'=>($combined?'required':'nullable').'|numeric|between:0,100','data.gradation_max_size'=>($combined?'required':'nullable').'|in:10,20,40','data.gradation_curve'=>($combined?'required':'nullable').'|integer|between:1,5','notes'=>'nullable']);
  $d['data']['construction_type']??=2;
 ...
 private function optimizeCombinedGradation(?AggregateTestRun $fineRun,?AggregateTestRun $coarseRun,int $maxSize,int $curve,?float $savedFinePercent=null):?array{
  ...
  $bestFine=null;$bestDeviation=INF;$candidates=$savedFinePercent===null?range(0,1000):[(int)round(max(0,min(100,$savedFinePercent))*10)];foreach($candidates as $tenths){$finePercent=$tenths/10;$coarsePercent=100-$finePercent;$sum=0;$count=0;foreach($activeKeys as $key){$target=$targets[$key];if($target===null)continue;$combined=$fine[$key]*$finePercent/100+$coarse[$key]*$coarsePercent/100;$sum+=abs($combined-$target);$count++;}if($count){$average=$sum/$count;if($average<$bestDeviation-0.0000001){$bestDeviation=$average;$bestFine=$finePercent;}}}
