...
class WorkflowController extends Controller {
 ...
 public function updateMixDesignSelection(Request $r,Project $project){
  ...
  $required=$method==='both'?['mix-design-2012','mix-design-2012-combined']:[$method];
  $available=LaboratoryWorkflow::where('project_id',$project->id)->whereIn('type',$required)->pluck('type')->unique();
  if($available->count()!==count($required))return back()->withErrors(['report_mix_design_method'=>'Metode yang dipilih belum mempunyai hasil perhitungan tersimpan.']);
  $before=$project->toArray();$project->update(['report_include_mix_design_2012'=>in_array('mix-design-2012',$required,true),'report_include_mix_design_2012_combined'=>in_array('mix-design-2012-combined',$required,true),'updated_by'=>auth()->id()]);
  AuditService::record('Laporan Proyek','ubah metode desain campuran laporan',$project,$before);
  return back()->with('success','Pilihan metode desain campuran untuk laporan berhasil disimpan.');
 }
 ...
 public function publicVerification(string $code){$project=Project::withTrashed()->where('verification_code',$code)->firstOrFail();$currentHash=$this->documentHash($project);$hashValid=(bool)($project->document_hash&&hash_equals($project->document_hash,$currentHash));$verificationStatus=$project->document_status==='direvisi'?'revisi':(($project->document_status==='valid'&&$hashValid)?'valid':'dokumen palsu');$latestApprovalRevision=$project->reportApprovals()->max('revision');$project->load(['reportApprovals'=>fn($q)=>$q->with('user')->when($latestApprovalRevision!==null,fn($query)=>$query->where('revision',$latestApprovalRevision))->when($verificationStatus==='valid',fn($query)=>$query->where('status','valid'))->orderByDesc('approved_at')->orderByDesc('id')]);$project->setRelation('reportApprovals',$project->reportApprovals->unique('approval_role')->values());return view('public.verification',compact('project','hashValid','verificationStatus'));}
 ...
 public function updateReportStatus(Request $r,Project $project){
  ...
  if($status==='revisi'){abort_unless($project->locked_at||$project->document_status==='ditolak',422,'Dokumen belum dapat dimasukkan ke revisi.');$project->reportApprovals()->where('status','valid')->update(['status'=>'direvisi','revoked_at'=>now()]);$project->increment('report_revision');$project->update(['document_status'=>'direvisi','locked_at'=>null,'status'=>'aktif','legalized_at'=>null]);$this->setRecordStatus($project,'draft');AuditService::record('Laporan Proyek','masuk revisi',$project);return back()->with('success','Laporan masuk revisi dan dapat diperbaiki untuk diajukan kembali.');}
  ...
  $hash=$this->documentHash($project);$approvalRole=$d['approval_role']??'menyetujui';$project->reportApprovals()->where('revision',$project->report_revision)->where('approval_role',$approvalRole)->where('status','valid')->update(['status'=>'direvisi','revoked_at'=>now()]);$approval=ReportApproval::create(['approval_id'=>Str::uuid(),'verification_token'=>Str::random(64),'project_id'=>$project->id,'user_id'=>auth()->id(),'revision'=>$project->report_revision,'approval_role'=>$approvalRole,'approval_type'=>'pengesahan laporan','status'=>'valid','document_hash'=>$hash,'ip_address'=>$r->ip(),'user_agent'=>Str::limit((string)$r->userAgent(),1000,''),'approved_at'=>now(),'notes'=>$d['notes']??null]);
