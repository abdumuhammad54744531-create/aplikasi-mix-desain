...
@else
@if(auth()->user()->hasPermission('dashboard.view'))<a class="nav-link {{request()->routeIs('dashboard')?'active':''}}" href="{{route('dashboard')}}"><i class="bi bi-grid me-2"></i>Dasbor</a>@endif
<small>DATA UTAMA</small>@if(auth()->user()->hasPermission('projects.view'))<a class="nav-link {{request()->routeIs('projects.*')?'active':''}}" href="{{route('projects.index')}}"><i class="bi bi-briefcase me-2"></i>Data Proyek</a>@endif
@if(auth()->user()->hasPermission('materials.view'))<a class="nav-link {{request()->routeIs('materials.*')?'active':''}}" href="{{route('materials.index')}}"><i class="bi bi-box-seam me-2"></i>Sumber Material</a>@endif
<small>PERENCANAAN</small>@if(auth()->user()->hasPermission('pasir.view')||auth()->user()->hasPermission('kerikil.view'))<a class="nav-link {{request()->routeIs('material-tests.*')?'active':''}}" href="{{route('material-tests.index')}}"><i class="bi bi-clipboard2-pulse me-2"></i>Pemeriksaan Material</a>@endif @if(auth()->user()->hasPermission('mix-design.view'))<a class="nav-link {{request()->routeIs('mix-design-2012.*')?'active':''}}" href="{{route('mix-design-2012.create')}}"><i class="bi bi-calculator me-2"></i>Desain Campuran 2012</a>
<a class="nav-link {{request()->routeIs('mix-design-2012-combined.*')?'active':''}}" href="{{route('mix-design-2012-combined.create')}}"><i class="bi bi-bezier2 me-2"></i>Desain Campuran 2012 (Gradasi Gabungan)</a>@endif
@if(auth()->user()->hasPermission('dokumentasi.view'))<a class="nav-link {{request()->routeIs('documentation.*')?'active':''}}" href="{{route('documentation.index')}}"><i class="bi bi-images me-2"></i>Dokumentasi</a>@endif
@foreach([
['Kuat Tekan','compressive-strength']
] as [$item,$slug])@if(auth()->user()->hasPermission('kuat-tekan.view'))<a class="nav-link {{request()->is('workflow/'.$slug)?'active':''}}" href="{{route('workflow.index',$slug)}}"><i class="bi bi-chevron-right me-2"></i>{{$item}}</a>@endif @endforeach
@if(auth()->user()->hasPermission('laporan.view'))<a class="nav-link {{request()->routeIs('workflow.reports')?'active':''}}" href="{{route('workflow.reports')}}"><i class="bi bi-chevron-right me-2"></i>Laporan</a>@endif
<small>ADMINISTRASI</small>
@if(auth()->user()->hasPermission('users.view'))<a class="nav-link {{request()->routeIs('accounts.*')?'active':''}}" href="{{route('accounts.index')}}"><i class="bi bi-people me-2"></i>Manajemen Pengguna</a>@endif @if(auth()->user()->hasPermission('standards.view'))<a class="nav-link {{request()->routeIs('jmd.standards.*')||request()->routeIs('jmd.standard-*')?'active':''}}" href="{{route('jmd.standards.index')}}"><i class="bi bi-table me-2"></i>Master Standar JMD</a>@endif @if(auth()->user()->hasPermission('report-settings.view'))<a class="nav-link {{request()->routeIs('report-settings.*')?'active':''}}" href="{{route('report-settings.edit')}}"><i class="bi bi-file-earmark-gear me-2"></i>Pengaturan Laporan</a>@endif
@if(auth()->user()->hasPermission('archive.view'))<a class="nav-link {{request()->routeIs('archive.*')?'active':''}}" href="{{route('archive.index')}}"><i class="bi bi-archive me-2"></i>Arsip</a>@endif
@if(auth()->user()->hasPermission('audit.view'))<a class="nav-link" href="{{route('audit')}}"><i class="bi bi-clock-history me-2"></i>Riwayat Audit</a>@endif
<small>PELAYANAN</small>@if(auth()->user()->hasPermission('requests.view'))<a class="nav-link {{request()->routeIs('lab-requests.*')?'active':''}}" href="{{route('lab-requests.index')}}"><i class="bi bi-file-earmark-text me-2"></i>Permohonan Uji Lab</a>@endif<a class="nav-link" href="{{route('lab-services.brochure')}}" target="_blank"><i class="bi bi-layout-text-window me-2"></i>Brosur Pengujian</a>
@endif
</nav></aside>
