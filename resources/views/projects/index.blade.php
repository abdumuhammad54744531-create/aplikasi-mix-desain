...
<tr><td><b>{{$p->name}}</b><div class="small text-secondary">{{$p->number}}</div></td><td>{{$p->owner??'—'}}</td><td>{{$p->location??'—'}}</td><td>{{$p->contract_number??'—'}}</td><td>{{$p->construction_type??'—'}}</td><td><div class="d-flex gap-1"><a class="btn btn-sm btn-outline-success" href="{{route('jmd.material-tests.index',$p)}}" title="Pengujian material JMD" aria-label="Pengujian material JMD"><i class="bi bi-flask"></i></a><button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editProject{{$p->id}}" title="Ubah proyek" aria-label="Ubah proyek"><i class="bi bi-pencil-square"></i></button><form method="post" action="{{route('projects.destroy',$p)}}" onsubmit="return confirm('Arsipkan proyek ini?')">@csrf @method('delete')<button class="btn btn-sm btn-outline-danger" title="Arsipkan" aria-label="Arsipkan proyek"><i class="bi bi-trash"></i></button></form></div></td></tr>
