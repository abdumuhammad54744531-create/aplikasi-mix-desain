...
.chart{width:100%;height:108mm;border:1px solid #aaa;margin:8px 0}.toc{font-size:var(--report-table-size)!important;line-height:1;table-layout:fixed}.toc td{padding:.5px 4px!important;vertical-align:middle}.info.toc td:first-child{width:92%!important;font-weight:600}.info.toc td:last-child{width:8%!important;text-align:right;white-space:nowrap}.toc tr{page-break-inside:avoid}.toc b{letter-spacing:0}.toc .toc-indent{padding-left:6mm!important;font-weight:400!important}.conclusion-table{font-size:7.4pt!important;line-height:1.08}.conclusion-table th,.conclusion-table td{padding:2px!important}
...
@forelse(($latestStrength->result_data['detail_rows']??[]) as $row)<tr><td>{{$row['number']}}</td><td>{{$row['cast_date']}}</td><td>{{$row['test_date']}}</td><td>{{$row['age_days']}}</td><td>{{$value($row['diameter'])}} / {{$value($row['height'])}}</td><td>{{$value($row['weight'])}}</td><td>{{$value($row['load_kn'])}}</td><td>{{$value($row['actual_mpa'])}}</td><td>{{$value($row['estimated_28_mpa'])}}</td><td>{{$value(isset($row['estimated_28_mpa'])?(float)$row['estimated_28_mpa']*10/0.83:null)}}</td></tr>@empty<tr><td colspan="10" class="center">Rincian benda uji belum tersedia.</td></tr>@endforelse
